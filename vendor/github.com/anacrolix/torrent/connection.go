...
func (cn *connection) Close() {
	if cn.pieceLimitQueue != nil {
		close(cn.pieceLimitQueue)
		cn.pieceLimitQueue = nil
	}
	if cn.requestLimitQueue != nil {
		close(cn.requestLimitQueue)
		cn.requestLimitQueue = nil
	}
	cn.closed.Set()
...
func (cn *connection) requestMetadataPiece(index int) {
	...
	cn.Post(pp.Message{
		...
		ExtendedPayload: func() []byte {
			...
			if err != nil {
				log.Printf("marshal RequestMetadataExtensionMsg error: %v", err)
			}
			return b
		}(),
	})
	...
	for {
		...
		for cn.outgoingUnbufferedMessages != nil && cn.outgoingUnbufferedMessages.Len() != 0 {
			...
			if msg.Type == pp.Piece {
				cn.t.pieceBufferPool.Put(msg.Piece[0:MaxChunkSize])
			}
...
func (cn *connection) Bitfield(haves []bool) {
	if cn.sentHaves != nil {
		log.Println("bitfield must be first have-related message sent")
		return
	}
	cn.Post(pp.Message{
		Type:     pp.Bitfield,
...
func (cn *connection) fillRequests() {
	cn.pieceRequestOrder.IterTyped(func(piece int) (more bool) {
		if cn.t.cl.config.Debug && cn.t.havePiece(piece) {
			log.Printf("piece %d exist!", piece)
			return true
		}
...
func (cn *connection) peerSentBitfield(bf []bool) error {
	cn.peerHasAll = false
	if len(bf)%8 != 0 {
		return fmt.Errorf("expected bitfield length divisible by 8")
	}
