...
func addrCompactIP(addr net.Addr) (string, error) {
	...
	if v4 := ip.To4(); v4 != nil {
		if len(v4) != 4 {
			return "", fmt.Errorf("invalid ipv4 address: %v", v4)
		}
...
func handshake(sock io.ReadWriter, ih *metainfo.Hash, peerID [20]byte, extensions peerExtensionBytes) (res handshakeResult, ok bool, err error) {
	...
	post := func(bb []byte) {
		...
		case postCh <- bb:
		default:
			log.Println("mustn't block while posting")
		}
	}
	...
	if conn.PeerExtensionBytes.SupportsExtended() && cl.extensionBytes.SupportsExtended() {
		conn.Post(pp.Message{
			...
			ExtendedPayload: func() []byte {
				...
				if err != nil {
					log.Printf("marshal initial message error: %v", err)
				}
				return b
			}(),
		})
...
func (cl *Client) sendChunk(t *Torrent, c *connection, r request) error {
	...
	//fmt.Printf("len(b)=%d, r.Length=%d\n", len(b), r.Length)
	n, err := t.readAt(b[0:r.Length], p.Offset()+int64(r.Begin))
	if n != int(r.Length) {
		if err == nil {
			return fmt.Errorf("Request len(%v) not equal to read len(%v)", r.Length, n)
		}
		return err
	}
	c.Post(pp.Message{
...
func (cl *Client) connectionLoop(t *Torrent, c *connection) error {
	...
	for {
		...
		case pp.Piece:
			...
			t.pieceBufferPool.Put(msg.Piece[0:MaxChunkSize])
		case pp.Extended:
		...
		case pp.Port:
			...
			if err != nil {
				log.Printf("resolve port error: %v", err)
				break
			}
			if msg.Port != 0 {
...
func (cl *Client) dropTorrent(infoHash metainfo.Hash) (err error) {
	...
	err = t.close()
	if err != nil {
		log.Printf("close torrent error: %v", err)
	}
...
func (cl *Client) SetUploadRateLimit(upload int) error {
	if upload > 0 {
		if cl.uploadRateLimit != nil {
			cl.uploadRateLimit.SetLimit(rate.Limit(upload))
		} else {
			return fmt.Errorf("Upload rate limiter is null")
		}
	}
	return nil
}

func (cl *Client) GetUploadRateLimit() (int, error) {
	if cl.uploadRateLimit != nil {
		val := cl.uploadRateLimit.Limit()
		return int(val), nil
	} else {
		return -1, fmt.Errorf("Upload rate limiter is null")
	}
}

func (cl *Client) SetDownloadRateLimit(download int) error {
	if download > 0 {
		if cl.downloadRateLimit != nil {
			cl.downloadRateLimit.SetLimit(rate.Limit(download))
		} else {
			return fmt.Errorf("Download rate limiter is null")
		}
	}
	return nil
}

func (cl *Client) GetDownloadRateLimit() (int, error) {
	if cl.downloadRateLimit != nil {
		val := cl.downloadRateLimit.Limit()
		return int(val), nil
	} else {
		return -1, fmt.Errorf("Download rate limiter is null")
	}
}
