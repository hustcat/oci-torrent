...
func announceHTTP(ar *AnnounceRequest, _url *url.URL, host string) (ret AnnounceResponse, err error) {
	...
	for _, peer := range ret.Peers {
		outs = append(outs, fmt.Sprintf("%s:%d", peer.IP.String(), peer.Port))
		i++
		if i >= 5 {
			break
		}
	}
	log.Printf("announce result, interval: %d, seeders: %d, leechers: %d, peers: %v", ret.Interval, ret.Seeders, ret.Leechers, outs)
	return
}
