...
func (t *Torrent) newMetadataExtensionMessage(c *connection, msgType int, piece int, data []byte) pp.Message {
	...
	if err != nil {
		log.Printf("marshal MetadataExtensionMessage error: %v", err)
	}
	return pp.Message{
		Type:            pp.Extended,
...
func (t *Torrent) announceStopRequest() tracker.AnnounceRequest {
	return tracker.AnnounceRequest{
...
func (t *Torrent) addConnection(c *connection) bool {
	...
	if len(t.conns) >= t.maxEstablishedConns {
		log.Printf("too many connections %v", len(t.conns))
		return false
	}
	t.conns = append(t.conns, c)
	if c.t != nil {
		log.Println("connection already associated with a torrent")
		return false
	}
